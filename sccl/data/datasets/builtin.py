...
from detectron2.data.datasets.cityscapes import (
    load_cityscapes_instances as load_cityscapes_native_instances,
)
from .cityscapes_foggy import load_cityscapes_instances as load_cityscapes_foggy_instances
...
_RAW_CITYSCAPES_CAR_SPLITS = {
    "cityscapes_fine_instance_seg_train_car": (
        "cityscapes/leftImg8bit/train/",
        "cityscapes/gtFine/train/",
    ),
    "cityscapes_fine_instance_seg_val_car": (
        "cityscapes/leftImg8bit/val/",
        "cityscapes/gtFine/val/",
    ),
    "cityscapes_fine_instance_seg_test_car": (
        "cityscapes/leftImg8bit/test/",
        "cityscapes/gtFine/test/",
    ),
}
_CITYSCAPES_CAR_CONTIGUOUS_ID = 2


def _keep_only_cityscapes_car(dataset_dicts):
    filtered = []
    for record in dataset_dicts:
        rec = dict(record)
        annos = []
        for anno in record.get("annotations", []):
            if int(anno.get("category_id", -1)) != _CITYSCAPES_CAR_CONTIGUOUS_ID:
                continue
            car_anno = dict(anno)
            car_anno["category_id"] = 0
            annos.append(car_anno)
...
def register_all_cityscapes_car(root):
    for key, (image_dir, gt_dir) in _RAW_CITYSCAPES_CAR_SPLITS.items():
        image_dir = os.path.join(root, image_dir)
        gt_dir = os.path.join(root, gt_dir)

        DatasetCatalog.register(
            key,
            lambda x=image_dir, y=gt_dir: _keep_only_cityscapes_car(
                load_cityscapes_native_instances(x, y, from_json=True, to_polygons=True)
            ),
        )
...
register_all_cityscapes_car(_root)
register_all_clipart(_root)
register_all_water(_root)
register_kitti("kitti_train", os.path.join(_root, "kitti"))
register_sim10k("sim10k_train", os.path.join(_root, "sim10k"))
