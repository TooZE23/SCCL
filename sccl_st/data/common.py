...
class AspectRatioGroupedSemiSupDataset(AspectRatioGroupedDataset):
    """
    Batch data that have similar aspect ratio together for semi-supervised learning.
    Handles two data streams (labeled and unlabeled) without augmentation crops.

    It assumes the underlying dataset produces dicts with "width" and "height" keys.
    It will then produce a tuple of two lists (labeled_batch, unlabeled_batch),
    all with similar aspect ratios.
    """
    ...
    def __init__(self, dataset, batch_size):
        ...
        Args:
            dataset: a tuple containing two iterable generators (labeled and unlabeled data).
               ...
               to batch data.
            batch_size: a tuple of two ints (batch_size_label, batch_size_unlabel).
        """
    ...
    def __iter__(self):
        ...
        for d_label, d_unlabel in zip(self.label_dataset, self.unlabel_dataset):
            ...
            if (
                ...
                yield (label_bucket[:], unlabel_bucket[:])
                del label_bucket[:]
                del unlabel_bucket[:]
