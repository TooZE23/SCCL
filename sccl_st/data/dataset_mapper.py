...
from sccl_st.data.detection_utils import build_strong_augmentation
...
class DatasetMapperTwoCropSeparate(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        image_for_pil = image_weak_aug
        if self.img_format == "BGR":
            image_for_pil = image_weak_aug[:, :, ::-1]
        image_pil = Image.fromarray(image_for_pil.astype("uint8"), "RGB")
        image_strong_aug = np.array(self.strong_augmentation(image_pil))
        if self.img_format == "BGR":
            image_strong_aug = image_strong_aug[:, :, ::-1]
