...
class DAobjGeneralizedRCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs, branch="main", target_flag=None):
        ...
        if branch == "unsup_data_weak":
            ...
            # Teacher pseudo-label generation: inference-style, no loss
            proposals_rpn, _ = self.proposal_generator(
                images, features, branch="target"
            )
            # ROI head inference (bypass training path)
            roi_features = [features[f] for f in self.roi_heads.box_in_features]
            box_features = self.roi_heads.box_pooler(
                roi_features, [x.proposal_boxes for x in proposals_rpn]
            )
            box_features = self.roi_heads.box_head(box_features)
            predictions = self.roi_heads.box_predictor(box_features)
            pred_instances, _ = self.roi_heads.box_predictor.inference(
                predictions, proposals_rpn
            )
            return {}, proposals_rpn, pred_instances, predictions

        if not self.training:
            return self.inference(batched_inputs)

        # ================================================================
        # Teacher-Student framework branches (single-image preprocessing)
        # ================================================================
        if branch in ("supervised", "supervised_target"):
            ...
            # Standard detection loss (RPN + ROI), no SCCL domain adaptation
            if self.proposal_generator is not None:
                proposals, proposal_losses = self.proposal_generator(
                    images, features, gt_instances, branch="source"
                )
            else:
            ...
            _, detector_losses = self.roi_heads(
                images, features, proposals, gt_instances, branch="sourceonly"
            )
        ...
        # ================================================================
        # Original SCCL branches (sourceonly, main, projection)
        # ================================================================
        source_label = 0
        target_label = 1
        ...
        if branch == "projection":
            ...
            loss_hinge = (
                F.hinge_embedding_loss(sim_tt_pos_1, target_similar, margin=0.5)
                + F.hinge_embedding_loss(sim_ss_pos_1, target_similar, margin=0.5)
                + F.hinge_embedding_loss(sim_tt_pos_2, target_similar, margin=0.5)
                + F.hinge_embedding_loss(sim_ss_pos_2, target_similar, margin=0.5)
                + F.hinge_embedding_loss(sim_st_neg_1, target_dissimilar, margin=0.5)
                + F.hinge_embedding_loss(sim_st_neg_2, target_dissimilar, margin=0.5)
            )
            ...
            ## source: generate proposals only (skip RPN loss, already computed in supervised branch)
            ## then run ROI contrast-only (skip detection loss, keep center updates + contrast)
            if self.proposal_generator is not None:
                proposals_s, _ = self.proposal_generator(images_s, features_s, branch="target")
            else:
                assert "proposals" in batched_inputs[0]
                proposals_s = [x["proposals"].to(self.device) for x in batched_inputs]

            _, roi_losses_s = self.roi_heads(
                images_s, features_s, proposals_s, gt_instances, branch="source_contrast_only"
            )
            ...
            ## target: contrastive learning only (no detection loss)
